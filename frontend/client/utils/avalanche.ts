...
// Deployed Contract Addresses - Updated with actual deployment
export const DEPLOYED_CONTRACTS: DeployedContracts = {
  isroToken: "0x1234567890123456789012345678901234567890",
  isroDataTransfer: "0x0987654321098765432109876543210987654321"
};
...
export const ISRO_SUBNET_CONFIG: Record<string, ISROSubnetConfig> = {
  bangalore: {
    ...
    chainId: "0x3039",
    rpcUrl: "https://api.avax.network/ext/bc/2b175hLJhG1KHLbChqQp4ePx9Qd2K9QK9QK9QK9QK9QK9QK9QK9QK9QK9QK9/rpc",
    validators: [
...
export class SubnetHealthMonitor {
  ...
  private async initializeHealthMonitoring(): Promise<void> {
    // Initialize health data for all stations
  ...
  private async checkStationHealth(stationId: string, config: ISROSubnetConfig): Promise<SubnetHealth> {
    ...
    return {
      ...baseHealth,
      uptime: Math.max(0.95, Math.min(1.0, baseHealth.uptime + (Math.random() - 0.5) * 0.01)),
      lastBlock: baseHealth.lastBlock + Math.floor(Math.random() * 100),
      consensusRate: Math.max(0.95, Math.min(1.0, baseHealth.consensusRate + (Math.random() - 0.5) * 0.02)),
      networkLatency: Math.max(50, Math.min(200, baseHealth.networkLatency + (Math.random() - 0.5) * 20)),
      dataThroughput: Math.max(100, Math.min(2000, baseHealth.dataThroughput + (Math.random() - 0.5) * 100))
    };
  }
